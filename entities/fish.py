...
class Fish:
    ...
    def _load_frames(self):
        ...
        self.frames = [
            pygame.transform.scale(
                pygame.image.load(f"assets/sprites/fishes/{self.fish_type}{i}.png").convert_alpha(),
                (self.size, self.size)
            )
    ...
    def _flee(self, dx, speed, anim):
        ...
        # toca o som apenas no começo do impulso
        if self.run_time == 0:
            self.alert_timer = 40

            if hasattr(self.game, "fish_flee_sound"):
                # para som anterior
                self.game.fish_flee_channel.stop()

                # toca novo
                self.game.fish_flee_channel.play(
                    self.game.fish_flee_sound
                )
